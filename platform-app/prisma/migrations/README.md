...
**Prod Yandex Managed PG (acp-postgres, БД `acp`) — applied 2026-04-22**

`_prisma_migrations` содержит обе записи (`20260101000000_baseline` +
`20260422150000_add_project_version`), `Project.version INTEGER NOT NULL
DEFAULT 0` добавлен, все существующие строки получили `version = 0`.
С этого момента любые новые миграции катятся обычным
`npx prisma migrate deploy`.

Инструкция ниже сохранена на случай, если потребуется выполнить то же
самое на новом окружении (stage / локальная копия / восстановление из
дампа).

## Что нужно сделать владельцу БД

Один раз при первом применении на новом окружении:
