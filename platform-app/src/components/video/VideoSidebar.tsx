...
import { Plus, Pencil, Trash2, Clapperboard, Home, ArrowLeft, Loader2, MoreHorizontal } from "lucide-react";
...
export function VideoSidebar({ projectId, projectName }: VideoSidebarProps) {
    ...
    const confirmDelete = () => {
        ...
        deleteSessionMutation.mutate({ id }, { onSettled: () => setDeleteTargetId(null) });
        if (id === activeSessionId) {
            setActiveSession(null);
        }
    };

    const formatSessionLabel = (s: { name: string | null; _count: { messages: number } }, index: number) => {
    ...
    return (
        <aside className="w-[220px] shrink-0 border-r border-border-primary bg-bg-surface flex flex-col min-h-0">
            <div className="px-4 py-3 border-b border-border-primary">
                ...
                <div className="text-[11px] text-text-tertiary mt-0.5">Видео-проект</div>
            </div>

            <div className="px-3 pt-3 pb-2">
                <button
                    onClick={() => createSessionMutation.mutate({ projectId })}
                    ...
                    sessions.map((s, index) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() => !isRenaming && setActiveSession(s.id)}
                            >
                                <Clapperboard size={12} className="shrink-0 text-text-tertiary" />
                                {isRenaming ? (
                                    <input
                                        autoFocus
                                        value={renameValue}
