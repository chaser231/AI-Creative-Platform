...
Этот документ описывает, как может выглядеть roadmap разработки canvas-редактора AI Creative Platform. Фокус не только на последовательности этапов, но и на том, как синхронизировать frontend и backend, чтобы команда не ждала друг друга и параллельно закладывала базу под будущие “умные” функции.
...
- Приоритет продукта: быстро дойти до полезного редактора, но сразу строить доменную модель так, чтобы она не мешала multi-format, smart resize и AI orchestration.
...
- template-driven композиция с frame/slot моделью;
- мультиформатная адапатация и экспорт;
- AI поверхности для генерации текста и изображений;
- продвинутый слой автоматизации: smart resize, семантический маппинг, AI-агентные действия.
...
1. На старте фиксируются доменные схемы:
   ...
   - `ResizeMapping`
   - `AIAction`
2. Frontend строит editor на mock и seeded fixtures.
