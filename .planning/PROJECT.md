...
- [x] Многослойный canvas (text / image / rectangle / badge / frame).
- [x] Master-component → instance cascade (форматные биндинги).
- [x] Resize с умным layout (AutoLayout, SmartResize).
- [x] История undo/redo (`createHistorySlice`, MF-6 in-progress — см. `.planning/mf6/DESIGN.md`).

### AI
- [x] Чат-агент с LLM tool-calling.
- [x] Генерация заголовков / субтитров / изображений / шаблонов.
- [x] VLM-анализ референсов (Gemini/OpenAI vision).

### Project & Storage
- [x] Проекты + версии + избранное.
- [x] Asset library + S3 upload с presign.
- [x] Template catalog.

### Auth & RBAC
- [x] Yandex OAuth + NextAuth v5.
- [x] Workspace-scoped RBAC (VIEWER < USER < CREATOR < ADMIN).
- [x] Waitlist / approved / suspended статусы.

### Integrations
- [x] Figma import (OAuth + mapper).
